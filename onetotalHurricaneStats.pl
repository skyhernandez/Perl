...
# @version    $Header$
# @cdaacTask  no
# @usage      onetotalHurricaneStats.pl storm_name mission
# @example    onetotalHurricaneStats.pl KENNETH champ
# -----------------------------------------------------------------------*/
EOD
...
my $matches_threshold = 1;

my $total = 0;

my @all_time_steps = ();
print "Occultations for $storm\n";
STORMROW:
...
my $args = {RESULTS   => \@all_time_steps,
            ...
            PLOTTITLE => "Ref: ($mission - GFS)/$mission, $storm"};
...
open(OUT,">$storm.$mission.gfs.txt");
