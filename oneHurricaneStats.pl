...
# @version    $Header$
# @cdaacTask  no
# @usage      oneHurricaneStats.pl storm_name mission
# @example    oneHurricaneStats.pl KENNETH champ
# -----------------------------------------------------------------------*/
EOD
...
foreach my $row (@$storm_rows) {
    ...
    my $args = {RESULTS   => $results,
	        ...
	        PLOT      => "$storm.$mission.$stamp.gfs.png/PNG",   # name of plot file
    ...
    open(OUT,">$storm.$mission.$stamp.gfs.txt");
